...
function customAssign(...objects) {
  ...
  const custAss = Object.assign({}, ...objects);
  // const combinedObject = Object.assign({}, ...objects);
  // Перетворення числових значень на рядки, ітеруємо об'єкт за допомогою for in
  for (const iter in custAss) {
    if (typeof custAss[iter] === "number") {
      custAss[iter] = String(custAss[iter]);
    }

    if (typeof custAss[iter] === "string") {
      custAss[iter] = custAss[iter].replace(/ /g, "_");
    }
  }
  ...
  //   if (typeof combinedObject[key] === "string") {
  //     combinedObject[key] = combinedObject[key].replace(/ /g, "_");
  //   }
  // }
...
function customEntries(obj) {
  ...
  // }
  const resultArray = Object.entries(obj);
  // Повернення масиву властивостей та
  return resultArray;
}
...
function customObjectFromEntries(entries) {
  ...
  const procEntries = entries.map(([key, value]) => {
  ...
  const resultObject = Object.fromEntries(procEntries);
...
console.log(extendAndFreezeObject({ a: 1, b: 2 }, { c: 3, d: 4 }));
...
function keysAndValues(obj) {
  ...
  const resultArr = [keyObj, valuesObj];
  return resultArr;
}
...
function filterObjectsByKey(arr, key) {
  ...
  const filterKey = arr.filter((obj) => obj.hasOwnProperty(key));
...
function convertArrayToObj(arr) {
  ...
  return Object.fromEntries(Object.entries(newObject));
}
